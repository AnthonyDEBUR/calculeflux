...
#'@param source : source des données : archives_dreal ou hub_eau
#' 
...
importe_debit <- function(X, Y, date_debut, date_fin, source="archives_dreal") {
  ...
  if (date_fin < date_debut) {
   ...
   if (!source %in% c("archives_dreal", "hub_eau")) {
    stop("source doit valoir soit archives_dreal soit hub_eau")
  }
...
#  dates_dispo<-dates_dispo[dates_dispo$name!=(" hub_eau"),]
  ...
  sources_data <- data.frame(source = source,
...
test_that("importe_debit_siva works",
  ...
  port = 3306
            ...
            debit_barrage <- importe_debit_siva(
                debut = as.POSIXct(strptime("2009-12-31 00:00:00", format = "%Y-%m-%d %H:%M:%S")),
...
```
# importe_stations_piezo_hubeau
    
```{r function-importe_stations_piezo_hubeau}
...
```{r example-importe_stations_piezo_hubeau}
 utils::data(sage_vilaine, package="calculeflux")
...
```{r tests-importe_stations_piezo_hubeau}
...
# importe_piezo_hubeau    
```{r function-importe_piezo_hubeau}
#' This function allows for importing available piezometric data from the Piezometrie 
#' HUB'EAU API (https://hubeau.eaufrance.fr/page/api-piezometrie).
#' 
#' @param code_bss vector with list of old bss code for piezometre (ex. 04193X0022/S2-6) 
...
```{r example-importe_piezo_hubeau}
